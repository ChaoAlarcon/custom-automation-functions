...
def move_file(path: Path, file: Path, extensions_dict: dict):
    ...
    for folder, extensions in extensions_dict.items():
        if file_ext in extensions:
            ...
            if dest.exists():
                logger.warning(f"Omitiendo '{file.name}' (ya existe en {folder}/)")
                moved = True
                break
    ...
    if not moved:
        ...
        dest = new_path / file.name
        if dest.exists():
            logger.warning(f"Omitiendo '{file.name}' (ya existe en others/)")
            return
...
def order_dir(dir_path: str):
    ...
    logger.info(f"Organización completada: {files_moved} archivos procesados")
